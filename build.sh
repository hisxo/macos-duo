...
swift Tools/CheckIcon.swift build
...
xcrun swiftc -O -swift-version 5 -target "${DUO_ARCH:-$(uname -m)}-apple-macosx14.0" -file-prefix-map "$PWD=." -debug-prefix-map "$PWD=." -file-compilation-dir . Sources/*.swift -o "$APP/Contents/MacOS/MacOSDuo" -framework SwiftUI -framework AppKit -framework MetalKit -framework MetalPerformanceShaders -framework IOKit -framework ScreenCaptureKit -framework Carbon -framework ServiceManagement
